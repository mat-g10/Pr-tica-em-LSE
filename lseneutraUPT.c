...
void* remover_final_lse(t_lse* lse){
    ...
    if(lse->inicio == lse->fim){ // Se há apenas um elemento
        void* carga = lse->inicio->carga_util;
        free(lse->inicio);
    ...
    while(cam->prox != NULL){ // Encontra o penúltimo elemento
        ant = cam;
        cam = cam->prox;
    }

    void* carga = cam->carga_util;
    free(cam);
...
int main(){
    // Exemplo de uso
    t_lse* lista = criar_lse(NULL, NULL); // Substitua por suas funções reais

    inserir_final_lse(lista, (void*)1);
    inserir_final_lse(lista, (void*)2);
    inserir_final_lse(lista, (void*)3);

    imprimir_lse(lista);

    //void* removido = remover_final_lse(lista);
